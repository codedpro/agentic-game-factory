...
const THEMES := {
	"classic": {
		...
		"tiles": {2: Color("4f8cf0"), 4: Color("f0964f"), 8: Color("ef5d7a"),
			16: Color("9b59d0"), 32: Color("4fc06a"), 64: Color("35b5b0"),
			128: Color("d4a017"), 256: Color("d84fd0"), 512: Color("e2633a"),
			1024: Color("30a8d8"), 2048: Color("f2c230")},
	},
	"sunset": {
		"bg": Color("241a26"), "panel": Color("342331"), "accent": Color("ff8c5a"),
		"tiles": {2: Color("ff8c5a"), 4: Color("f95f78"), 8: Color("d6486f"),
			16: Color("a83a72"), 32: Color("f7b267"), 64: Color("f4845f"),
			128: Color("c04ca0"), 256: Color("8e44ad"), 512: Color("ffd166"),
			1024: Color("ef476f"), 2048: Color("ffe08a")},
	},
	"neon": {
		"bg": Color("0a0e14"), "panel": Color("131a24"), "accent": Color("00e5ff"),
		"tiles": {2: Color("00e5ff"), 4: Color("39ff88"), 8: Color("ff2975"),
			16: Color("b026ff"), 32: Color("ffe600"), 64: Color("ff6b1a"),
			128: Color("00ffc8"), 256: Color("ff2ec4"), 512: Color("7dff00"),
			1024: Color("00a2ff"), 2048: Color("ffd700")},
	},
	"garden": {
		"bg": Color("15241c"), "panel": Color("1f3328"), "accent": Color("6fcf6f"),
		"tiles": {2: Color("6fcf6f"), 4: Color("a3c957"), 8: Color("e2b458"),
			16: Color("d98950"), 32: Color("57b8a0"), 64: Color("4b9560"),
			128: Color("c9a227"), 256: Color("8f6bbf"), 512: Color("e07a5f"),
			1024: Color("3d9bb3"), 2048: Color("f2cc5d")},
	},
}
const TILE_COLOR_HUGE := Color("47506a")
const STONE_COLOR := Color("3a3f4d")
...
func board_metrics() -> Dictionary:
	var v := vp()
	var gap := 8.0
	var top_h: float = clampf(v.y * 0.155, 140, 215)
	var bottom_h: float = clampf(v.y * 0.16, 150, 225)
	var tile_w := (v.x - 32 - gap * 4) / 5.0
	var tile_h := (v.y - top_h - bottom_h - gap * 6) / 7.0
	var tile := floorf(minf(tile_w, tile_h))
	var bw := 5 * tile + 4 * gap
	return {"tile": tile, "gap": gap, "x0": (v.x - bw) / 2.0, "top": top_h,
...
## Game tile with themed color, bevel border, glow for high values; STONE = -1.
func tile(value: int, size: float, alpha := 1.0) -> Panel:
	var col: Color = STONE_COLOR if value == Board.STONE \
		else theme().tiles.get(value, TILE_COLOR_HUGE)
	var p := Panel.new()
	var sb := StyleBoxFlat.new()
	sb.bg_color = col
	sb.set_corner_radius_all(int(size * 0.145))
	sb.set_border_width_all(maxi(2, int(size * 0.025)))
	sb.border_color = col.lightened(0.22) if value != Board.STONE else col.darkened(0.2)
	if value >= 128:
	...
	if value == Board.STONE:
		var l := label("✖", int(size * 0.4), true, Color("6b7183"))
		...
		if n == 3:
			...
			fs = 0.28
		var l := label(I18n.digits(value), int(size * fs))
