extends Node
## Autoload "Store" — persistent settings, records, achievements, economy, fal collection.
...
var fal_collected: Array = []    # poem ids unlocked in the treasury
var fal_last_date := ""          # yyyymmdd of last daily fal received
var fal_log: Array = []          # [{"id","d":"yyyy-mm-dd","t":topic_key}] newest last
...
func load_data() -> void:
	...
	fal_collected = cf.get_value("game", "fal_collected", [])
	fal_last_date = cf.get_value("game", "fal_last_date", "")
	fal_log = cf.get_value("game", "fal_log", [])
...
func save() -> void:
	...
	cf.set_value("game", "fal_collected", fal_collected)
	cf.set_value("game", "fal_last_date", fal_last_date)
	cf.set_value("game", "fal_log", fal_log)
...
func collect_fal(id: String) -> bool:
	if id in fal_collected:
		return false
	fal_collected.append(id)
	save()
	return true
