...
func _build_coins(col: VBoxContainer, v: Vector2, fs: int) -> void:
	...
	var note := UI.label(I18n.t("iap_note") % IAP.store_name(), fs - 3, false, UI.MUTED)
...
func _toast(txt: String, bad := false) -> void:
	...
	_msg.z_index = 20
	add_child(_msg)
	# tween bound to the label: it dies with the node, so a freed screen can never
	# fire a callback with a dead capture (a scene-tree timer kept doing exactly that)
	var tw := _msg.create_tween()
	tw.tween_interval(1.6)
	tw.tween_callback(_msg.queue_free)
