...
func test_proverb_text_has_glyphs():
	for l in Masal.levels:
		var s: String = str(l.text) + str(l.meaning)
		for r in l.rounds:
			for w in r.wheel:
				s += str(w)
		for i in s.length():
			...
			assert_true(_has_glyph(UI.font_bold, ch),
				"level %s has unrenderable char '%s' (U+%X)" % [l.id, ch, ch.unicode_at(0)])


func test_dictionary_words_have_glyphs():
	for w in Masal.word_set:
		var word: String = str(w)
		for i in word.length():
			var ch: String = word[i]
			assert_true(_has_glyph(UI.font_bold, ch),
				"dictionary word %s has unrenderable char (U+%X)" % [word, ch.unicode_at(0)])
